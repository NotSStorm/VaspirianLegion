...
const statusStyles: Record<string, string> = {
  ...
  Scheduled: 'border-tan/40 bg-tan/10 text-tan',
  Completed: 'border-slateBlue/60 bg-slateBlue/20 text-slate-300'
};

export default function BattleCard({ name, classification, status, theater, commandingOfficer, personnelCount, date, pointsScored, threatLevel }: BattleCardProps) {
  const resolvedPoints = pointsScored ?? threatLevel ?? 0;
    ...
    <div className="rounded border border-slateBlue/60 bg-[#141a24] p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2 text-xs uppercase tracking-[0.3em]">
          ...
          {status && <span className={`rounded border px-2 py-1 ${statusStyles[status] ?? 'border-slateBlue/60 bg-slateBlue/20 text-slate-300'}`}>{status}</span>}
        </div>
      </div>
      <div className="mt-4 grid gap-3 text-sm text-slate-300 sm:grid-cols-2">
        {theater && <div><span className="text-slate-400">Theater</span><div className="font-semibold text-silver">{theater}</div></div>}
        ...
        <div><span className="text-slate-400">Points Scored</span><div className="font-semibold text-silver">{resolvedPoints}</div></div>
      </div>
    </div>
  );
}
